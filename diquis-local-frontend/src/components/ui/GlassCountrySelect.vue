...
import { ChevronDown, Globe, Search } from 'lucide-vue-next';
...
const selectedCountry = computed(() => {
  return COUNTRIES.find(c => c.code === props.modelValue?.toLowerCase()) || null;
});
  ...
  <div ref="dropdownRef" class="relative w-full">
    ...
    >
      <div class="flex items-center gap-3">
        <div v-if="!selectedCountry" class="p-1.5 rounded-md bg-slate-200/50 dark:bg-white/10 text-slate-500 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
          <Globe class="w-4 h-4" />
        </div>
        ...
        <span v-if="!selectedCountry" class="text-sm text-slate-400 dark:text-slate-500 leading-none mt-[1px]">
          {{ placeholder || 'País...' }}
        </span>
      </div>
    ...
    <transition name="dropdown">
      <div v-if="isOpen" class="absolute z-50 w-full mt-2 p-2 rounded-2xl bg-white/95 dark:bg-[#111111]/95 backdrop-blur-2xl border border-white/50 dark:border-white/10 shadow-[0_20px_40px_rgb(0,0,0,0.15)] dark:shadow-[0_20px_40px_rgb(0,0,0,0.6)]">
        ...
        <div class="max-h-60 overflow-y-auto custom-scrollbar pr-1">
          <div v-if="filteredCountries.length === 0" class="py-4 text-center text-sm text-slate-500">
            Sin resultados.
          </div>
          ...
          <button
            ...
            :class="{ 'bg-slate-100 dark:bg-white/10': modelValue === country.code }"
          >
            <div class="flex items-center gap-3">
              <img 
              ...
              <div class="flex flex-col">
                <span class="text-sm font-bold text-slate-900 dark:text-white leading-none">{{ country.name }}</span>
              </div>
            </div>
            <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-black/30 px-1.5 py-0.5 rounded-md uppercase">
              {{ country.code }}
            </span>
          </button>
        </div>
